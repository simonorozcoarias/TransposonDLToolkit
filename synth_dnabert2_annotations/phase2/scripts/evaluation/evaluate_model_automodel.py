...
def evaluate_by_species(model, tokenizer, dataset, device, batch_size: int = 192) -> Dict[str, Dict]:
    """
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=192,
        help="Batch size for evaluation (default: 192 for A100 40GB - optimized)"
    )
    parser.add_argument(
        "--by_species",
        action="store_true",
    ...
    # Load model using BertForTokenClassification (matches training script)
    logger.info(f"Loading BertForTokenClassification from: {args.model_path}")

    # Note: Using BertForTokenClassification directly (same as train_token_classification_automodel.py)
    # This ensures compatibility with DNABERT-2's custom configuration class.
