#!/bin/bash
#SBATCH --job-name=dnabert2_debug
#SBATCH --output=logs/slurm_debug_%j.out
#SBATCH --error=logs/slurm_debug_%j.err
...
################################################################################
# DNABERT-2 DEBUG Script - Single Species Test
#
...
NUM_SPECIES=2  # Change this to test with multiple species
...
if [ $NUM_SPECIES -eq 1 ]; then
    ...
    OUTPUT_DIR="$PROJECT_DIR/results/dnabert2_debug_single_species"
    RUN_NAME="dnabert2_debug_single_species"
    ...
    OUTPUT_DIR="$PROJECT_DIR/results/dnabert2_debug_${NUM_SPECIES}_species"
    RUN_NAME="dnabert2_debug_${NUM_SPECIES}_species_${TIMESTAMP}"

    # Update output paths in config file
    echo "Updating output directories in config..."
    sed -i "s|output_dir:.*|output_dir: \"./results/dnabert2_debug_${NUM_SPECIES}_species\"|" "$CONFIG_FILE"
    sed -i "s|logging_dir:.*|logging_dir: \"./results/dnabert2_debug_${NUM_SPECIES}_species/logs\"|" "$CONFIG_FILE"
...
python -u scripts/train_token_classification.py \
    --config $CONFIG_FILE \
    $IMBALANCE_RATIO_ARG \
...
if [ $NUM_SPECIES -gt 1 ]; then
    ...
    sed -i 's|output_dir:.*|output_dir: "./results/dnabert2_debug_single_species"|' "$CONFIG_FILE"
    sed -i 's|logging_dir:.*|logging_dir: "./results/dnabert2_debug_single_species/logs"|' "$CONFIG_FILE"
    sed -i 's|run_name:.*|run_name: "dnabert2_debug_single_species"|' "$CONFIG_FILE"
