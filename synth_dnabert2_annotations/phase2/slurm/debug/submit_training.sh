#!/bin/bash
#SBATCH --job-name=dnabert2_te_finetune
...
################################################################################
# DNABERT-2 Token Classification Fine-tuning - SLURM Job Script
#
# This script submits a fine-tuning job for DNABERT-2 on a SLURM cluster.
...
echo "=================================="
echo "DNABERT-2 Fine-tuning Job Started"
echo "=================================="
...
TB_LOGDIR="results/dnabert2_te_token_classification/logs"
mkdir -p $TB_LOGDIR
...
stdbuf -oL -eL torchrun \
    ...
    scripts/train_token_classification.py \
    --config $CONFIG_FILE \
    $DEBUG_MODE
...
if [ $TRAINING_EXIT_CODE -eq 0 ]; then
    ...
    echo "  3. Evaluate on test set: python evaluate_model.py --model_path results/dnabert2_te_token_classification"
    echo ""
else
    echo ""
    echo "=================================="
